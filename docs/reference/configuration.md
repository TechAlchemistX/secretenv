...
2. No backend URIs; use `secretenv://` aliases or static defaults only.
3. One `[secrets]` block; no environment blocks.
4. Safe to commit. Zero infrastructure information.
...
- `[registries.default]` is optional; missing registry name is a hard error.
- Each registry has a `sources` array. Sources cascade. First match wins.
- Earlier sources shadow later ones by design.
