...
**Set-site enforcement (SEC-INV-04).** Every ALLOW attribute has exactly one method on `SecretEnvSpan` (e.g. `set_alias_name`, `set_backend_type`). The builder does **not** expose a generic `set_attribute(key: &str, value: &str)`. Adding a new attribute requires adding a new typed setter, which is a PR-reviewed code change. A CI grep gate (`scripts/check_tracing_leaks.sh`) fails the build if any call site references `Secret::expose_secret`, `{value}`, `{uri.raw}`, or `{secret}` inside a `tracing::*!` macro argument list.
